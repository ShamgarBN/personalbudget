...
pub fn list_recurring_bills(state: State<AppState>) -> AppResult<Vec<RecurringBill>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, name, amount, account_id, category_id, cadence_kind, day_of_month, anchor_date, active, last_seen_date, notes, start_date, end_date \
...
pub fn upsert_recurring_bill(state: State<AppState>, bill: RecurringBill) -> AppResult<i64> {
    ...
    if bill.id == 0 {
        conn.execute(
            "INSERT INTO recurring_bill (name, amount, account_id, category_id, cadence_kind, day_of_month, anchor_date, active, last_seen_date, notes, start_date, end_date) \
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            rusqlite::params![
                bill.name, bill.amount, bill.account_id, bill.category_id, bill.cadence_kind,
                bill.day_of_month, bill.anchor_date, bill.active as i64, bill.last_seen_date, bill.notes,
        ...
        conn.execute(
            "UPDATE recurring_bill \
                SET name=?, amount=?, account_id=?, category_id=?, cadence_kind=?, day_of_month=?, anchor_date=?, active=?, last_seen_date=?, notes=?, start_date=?, end_date=? \
              WHERE id=?",
            rusqlite::params![
                bill.name, bill.amount, bill.account_id, bill.category_id, bill.cadence_kind,
                bill.day_of_month, bill.anchor_date, bill.active as i64, bill.last_seen_date, bill.notes,
